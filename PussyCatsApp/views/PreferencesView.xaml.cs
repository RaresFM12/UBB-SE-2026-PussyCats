...
namespace PussyCatsApp.Views
{
    ...
    public sealed partial class PreferencesView : Page
    {
        ...
        public PreferencesView()
        {
            ...
            viewModel = new PreferencesViewModel(userId: 1);
        ...
        private void PopulateUIFromViewModel()
        {
            ...
            RolesListView.SelectionChanged += RolesListView_SelectionChanged; // restore handler
        ...
        {
            // Reflect added items into the ViewModel
            ...
            // Reflect removed items into the ViewModel
            ...
            // If the ViewModel rejected the add (max 3 reached), sync the UI back
            string error = viewModel.GetErrorMessage();
            if (!string.IsNullOrEmpty(error))
            {
                RoleWarningText.Visibility = Visibility.Visible;

                // Remove items the ViewModel rejected (were added but ViewModel refused them)
                RolesListView.SelectionChanged -= RolesListView_SelectionChanged;
                foreach (var item in e.AddedItems.Cast<string>())
                {
                    var role = roleDisplayNames.First(kv => kv.Value == item).Key;
        ...
        {
            // Push current UI state into ViewModel before saving
            ...
            // Validate that the user filled in all fields before delegating to ViewModel
